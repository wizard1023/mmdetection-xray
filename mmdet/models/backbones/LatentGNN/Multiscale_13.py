...
class Multiscale_13(nn.Module):
    def __init__(self):
        super(Multiscale_13, self).__init__()
        ...
        self.dilated_conv_3_3 = nn.Sequential(
            nn.Conv2d(1024, 512, kernel_size=3, stride=2, padding=2, dilation=2),
            nn.BatchNorm2d(512),
            nn.ReLU())
        ...
        self.weight_levels_3 = nn.Conv2d(16 * 2, 2, kernel_size=1, stride=1, padding=0)
        ...
        self.weight_levels_2 =nn.Conv2d(16 * 3, 3, kernel_size=1, stride=1, padding=0)
        ...
        self.weight_levels_1 = nn.Conv2d(16 * 3, 3, kernel_size=1, stride=1, padding=0)
        ...
        #     nn.ReLU())
        #
        # self.weight_levels_0 = nn.Conv2d(16 * 2, 2, kernel_size=1, stride=1, padding=0)



    def forward(self,feat):
        level_0,level_1,level_2_origin,level_3_origin = feat[0],feat[1],feat[2],feat[3]

        # for level_3
        level_3 = self.down_ch_3(level_3_origin)
        level_2_d_3 = self.dilated_conv_3_3(level_2_origin)
        ...
        fused_out_3 = self.expand_3(fused_out_3)

        # for level_2
        level_2 = self.down_ch_2(level_2_origin)
        level_3_u_2 = F.interpolate(self.conv_1(level_3_origin), scale_factor=2, mode='nearest')
        ...
        fused_out_2 = self.expand_2(fused_out_2)

        # for level_1
        # level_3_u_1 = F.interpolate(self.conv_2(level_3), scale_factor=4, mode='nearest')
        level_2_u_1 = F.interpolate(self.conv_3(level_2_origin), scale_factor=2, mode='nearest')
