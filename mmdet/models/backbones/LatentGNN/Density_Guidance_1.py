...
from mmdet.models.backbones.LatentGNN.MGL import GraphNet,BasicConv2d,MutualModule0


class Density_Guidance_1(nn.Module):
    def __init__(self):
        super(Density_Guidance_1, self).__init__()
        ...
        #     nn.BatchNorm2d(2048),
        #     nn.ReLU())
        self.rgb_down_ch = nn.Sequential(
    ...
    def forward(self, im, feat):
        ...
        hsv_x = self.conv_2048(hsv_1024)
        rgb_x = self.rgb_down_ch(feat[3])

        rgb_x = self.mgl(rgb_x, hsv_x)
        rgb_x =self.expand(rgb_x)
        return [feat[0],feat[1],feat[2],rgb_x]
        ...
        # outs_att_1 = self.latent_gnn_ch1(outs_1) * outs[0] + outs[0]
        #
        # outs_2 = self.maxpool2(outs[1])
        # outs_att_2 = self.latent_gnn_ch2(outs_2) * outs[1] + outs[1]
        #
        # outs_3 = self.maxpool3(outs[2])
        # outs_att_3 = self.latent_gnn_ch3(outs_3) * outs[2] + outs[2]
        #
        # outs_4 = self.maxpool4(outs[3])
        # outs_att_4 = self.latent_gnn_ch4(outs_4) * outs[3] + outs[3]
        #
...
class MGL_hsv(nn.Module):
    def __init__(self,BatchNorm=nn.BatchNorm2d, dim=512, num_clusters=8, dropout=0.1):
        super(MGL_hsv, self).__init__()
        self.dim = dim

        self.rgb_proj0 = GraphNet(node_num=num_clusters, dim=self.dim, normalize_input=False)
        self.hsv_proj0 = GraphNet(node_num=num_clusters, dim=self.dim, normalize_input=False)

        self.rgb_conv = nn.Sequential(BasicConv2d(self.dim, self.dim, BatchNorm, kernel_size=1, padding=0))
        # BasicConv2d(self.dim, self.dim, BatchNorm, kernel_size=3, padding=1)
        self.rgb_conv[0].reset_params()

        self.hsv_conv1 = nn.Sequential(BasicConv2d(self.dim, self.dim, BatchNorm, kernel_size=1, padding=0))
        self.hsv_conv1[0].reset_params()

        self.hsv_conv2 = nn.Sequential(BasicConv2d(self.dim, self.dim, BatchNorm, kernel_size=1, padding=0))
        self.hsv_conv2[0].reset_params()

        self.r2e = MutualModule0(self.dim, BatchNorm, dropout)

    def forward(self,rgb_x, hsv_x):
        rgb_graph, rgb_assign = self.rgb_proj0(rgb_x)
        hsv_graph, _ = self.hsv_proj0(hsv_x)
        rgb_graph = self.rgb_conv(rgb_graph.unsqueeze(3)).squeeze(3)
        # rgb-hsv mutual learning
        hsv_graph1 = self.hsv_conv1(hsv_graph.unsqueeze(3)).squeeze(3)
        hsv_graph2 = self.hsv_conv2(hsv_graph.unsqueeze(3)).squeeze(3)
        # CGI
        n_rgb_x = self.r2e(rgb_graph, hsv_graph1, hsv_graph2, rgb_assign)
...
if __name__=='__main__':
    ...
    model = Density_Guidance_1().to("cuda:2")
    outs = model(img, feat)
