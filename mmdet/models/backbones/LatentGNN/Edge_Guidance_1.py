...
class Edge_Guidance_1(nn.Module):
    def __init__(self):
        ...
        self.block2 = nn.Sequential(
            ...
            Block(5, 24, 96, 40, nn.ReLU(inplace=True),
                    LatentGNNV1(in_channels=40,latent_dims=[100,100],
                                channel_stride=4,num_kernels=2,mode='asymmetric',
                                graph_conv_flag=False),2))
        self.block3 = nn.Sequential(
            Block(3, 40, 120, 40, nn.ReLU(inplace=True),None,1),
            Block(3, 40, 240, 80, nn.ReLU(inplace=True),
                  LatentGNNV1(in_channels=80, latent_dims=[100, 100],
                              channel_stride=4, num_kernels=2, mode='asymmetric',
                              graph_conv_flag=False), 2))
        self.block4 = nn.Sequential(
            Block(3, 80, 200, 80, nn.ReLU(inplace=True), None, 1),
            Block(3, 80, 480, 160, nn.ReLU(inplace=True),
...
if __name__ == "__main__":
    ...
    model = Edge_Guidance_1().to("cuda:0")
    outs = model(img,feat)
    # edge_detect = edge_detect.squeeze(0).cpu().numpy()
    # image = np.transpose(edge_detect, (1, 2, 0))
    # cv2.imwrite("/home/xray/LXM/mmdetection/demo/edge_00151.jpg", image)
