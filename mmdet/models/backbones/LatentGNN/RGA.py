...
class RGA_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_channel:
            # channel attention
            xc = x.view(b, c, -1).permute(0, 2, 1).unsqueeze(-1) # 4,900,1024,1
            theta_xc = self.theta_channel(xc).squeeze(-1).permute(0, 2, 1) # 4,1024,225
            phi_xc = self.phi_channel(xc).squeeze(-1)  # 4,225,1024
            Gc = torch.matmul(theta_xc, phi_xc) # 4,1024,1024
            Gc_in = Gc.permute(0, 2, 1).unsqueeze(-1) # 4,1024,1024,1
            Gc_out = Gc.unsqueeze(-1) # 4,1024,1024,1
            Gc_joint = torch.cat((Gc_in, Gc_out), 1) # 4 ,2048,1024,1
            Gc_joint = self.gg_channel(Gc_joint) # 4,128,1024,1

            g_xc = self.gx_channel(xc)  # 4,225,1024,1
            g_xc = torch.mean(g_xc, dim=1, keepdim=True)  # 4,1,1024,1
            yc = torch.cat((g_xc, Gc_joint), 1) # 4,129,1024,1

            W_yc = self.W_channel(yc).transpose(1, 2) # 4,1024,1,1
            out = F.sigmoid(W_yc) * x

            return out

if __name__ == "__main__":
    inputs = torch.rand((4, 1024, 30, 30))
    model = RGA_Module(in_channel=1024,in_spatial=900, use_spatial= False,spa_ratio=4)
