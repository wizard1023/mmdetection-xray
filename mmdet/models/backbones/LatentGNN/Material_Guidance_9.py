...
from mmdet.models.backbones.LatentGNN.CGA import CGA

class Material_Guidance_9(nn.Module):
    def __init__(self):
        super(Material_Guidance_9, self).__init__()
        ...
        self.cga = nn.ModuleList([
            CGA(dim=256, output_size_h=320, output_size_w=200),
            CGA(dim=512, output_size_h=160, output_size_w=100),
            CGA(dim=1024, output_size_h=80, output_size_w=50),
            CGA(dim=2048, output_size_h=40, output_size_w=25)
        ])
...
if __name__=='__main__':
    ...
    model = Material_Guidance_9().to("cuda:2")
    outs = model(img, feat)
