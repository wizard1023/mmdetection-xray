...
class Multiscale_19(nn.Module):
    def __init__(self):
        super(Multiscale_19, self).__init__()

        # for level_3
        self.dilated_conv_3_1 = nn.Conv2d(256, 2048,kernel_size=3,stride=2,padding=1)
        self.dilated_conv_3_2 = nn.Conv2d(512, 2048, kernel_size=3, stride=2, padding=1)
        self.dilated_conv_3_3 = nn.Conv2d(1024, 2048, kernel_size=3, stride=2, padding=1)
        ...
        self.dilated_conv_2_1 = nn.Conv2d(512, 1024, kernel_size=3, stride=2, padding=1)
        self.dilated_conv_2_2 = nn.Conv2d(256,1024, kernel_size=3, stride=2, padding=1)
        ...
        self.dilated_conv_1_1 = nn.Conv2d(256,512, kernel_size=3, stride=2, padding=1)
...
if __name__=='__main__':
    img = torch.rand(2, 3, 640, 640).to("cuda:4")
    feat = [torch.rand(2, 256, 160, 160).to("cuda:4"),
            torch.rand(2, 512, 80, 80).to("cuda:4"),
            torch.rand(2, 1024, 40, 40).to("cuda:4"),
            torch.rand(2, 2048, 20, 20).to("cuda:4")]
    model = Multiscale_19().to("cuda:4")
    outs = model(feat)
