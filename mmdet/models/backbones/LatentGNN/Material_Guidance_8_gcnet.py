...
from mmdet.models.backbones.LatentGNN.GCNet import ContextBlock

class Material_Guidance_8_GCNet(nn.Module):
    def __init__(self):
        super(Material_Guidance_8_GCNet, self).__init__()
        ...
        self.latent_gnn_ch1 = ContextBlock(inplanes=256,ratio=2)

        # self.maxpool2 = nn.AdaptiveMaxPool2d(output_size=(160, 100))
        self.latent_gnn_ch2 = ContextBlock(inplanes=512,ratio=2)

        # self.maxpool3 = nn.AdaptiveMaxPool2d(output_size=(80, 50))
        self.latent_gnn_ch3 = ContextBlock(inplanes=1024,ratio=2)

        # self.maxpool4 = nn.AdaptiveMaxPool2d(output_size=(40, 25))
        self.latent_gnn_ch4 = ContextBlock(inplanes=2048,ratio=2)
...
if __name__=='__main__':
    ...
    model = Material_Guidance_8_GCNet().to("cuda:2")
    outs = model(img, feat)
