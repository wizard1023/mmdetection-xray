...
class Edge_Guidance_4(nn.Module):
    def __init__(self):
        super(Edge_Guidance_4, self).__init__()
        # horizontal
        ...
        # stem
        self.conv1 = nn.Conv2d(1, 128, kernel_size=6, stride=2, padding=2, bias=False) # /2
        self.bn1 = nn.BatchNorm2d(128)
        self.relu1 = nn.ReLU6(inplace=True)

        self.block1 = nn.Sequential(
            #Block(3, 128, 256, 128, nn.ReLU6(inplace=True), None, 1),
            Block(3, 128, 256, 256, nn.ReLU6(inplace=True), None, 2))

        self.block2 = nn.Sequential(
            #Block(3, 256, 512, 256, nn.ReLU6(inplace=True), None, 1),
            Block(5, 256, 512, 512, nn.ReLU6(inplace=True),
                    LatentGNNV1(in_channels=512,latent_dims=[100,100],
                                channel_stride=8,num_kernels=2,mode='asymmetric',
                                graph_conv_flag=False),2))
        self.block3 = nn.Sequential(
            #Block(3, 512, 1024, 512, nn.ReLU6(inplace=True),None,1),
            Block(3, 512, 1024, 1024, nn.ReLU6(inplace=True),
                  LatentGNNV1(in_channels=1024, latent_dims=[100, 100],
                              channel_stride=8, num_kernels=2, mode='asymmetric',
                              graph_conv_flag=False), 2))
        self.block4 = nn.Sequential(
            #Block(3, 1024, 2048, 1024, nn.ReLU6(inplace=True), None, 1),
            Block(3, 1024, 2048, 2048, nn.ReLU6(inplace=True), None, 2))
        self.init_params()
...
if __name__ == "__main__":
    ...
    model = Edge_Guidance_4().to("cuda:0")
    outs = model(img,feat)
    # edge_detect = edge_detect.squeeze(0).cpu().numpy()
    # image = np.transpose(edge_detect, (1, 2, 0))
    # cv2.imwrite("/home/xray/LXM/mmdetection/demo/edge_00151.jpg", image)
